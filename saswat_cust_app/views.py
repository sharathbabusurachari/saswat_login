        ...
        try:
            ...
            if UserDetails.objects.filter(mobile_no=mobile_no).exists():
                ...
                if response.status_code == 200:
                    ...
                    return Response({'message': 'Failed to send OTP to the user.'},
        ...
        if serializer.is_valid():
            ...
            if UserDetails.objects.filter(mobile_no=mobile_no).exists():
                ...
                if verify_user_otp:
                    ...
                    return Response({'session_id': session_id, 'success': 'OTP verified successfully'}, status=200)
                elif valid_otp_time:
                    ...
                    return JsonResponse({'error': 'Invalid OTP'}, status=status.HTTP_401_UNAUTHORIZED)
            else:
                return JsonResponse({'Detail': 'Mobile number does not exist'}, status=status.HTTP_400_BAD_REQUEST)
